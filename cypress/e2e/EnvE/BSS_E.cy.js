...
cy.get('div[class="sc-quote-data sc-subscription-select"]').click();
   ...
   cy.get('#alternateCardRadio').check({force: true}) .check()
    ...
    cy.writeFile('../Weekly_Smoke_Test_Report_Env E.html','<p> ENVIRONMENT E :</p>', { flag: 'a+' } )

      { flag: 'a+' } 

    cy.writeFile('../Weekly_Smoke_Test_Report_Env E.html', '<p>------------------------------------------------</p>' + '\r\n' , {flag: 'a+'} )

    cy.writeFile('../Weekly_Smoke_Test_Report_Env E.html', '<p>BSS Checks:</p>'  + '\r\n' , {flag: 'a+'})
...
cy.get('body').then(($body) => {
      ...
      {
        cy.writeFile('../Weekly_Smoke_Test_Report_Env E.html', '<p>1. Account Creation and Purchasing a product: Transaction is failed as a new Member.<span style="color: red;">Test is Failed.</span></p>'  + '\r\n' , {flag: 'a+'})
      }
      else
      {
        cy.writeFile('../Weekly_Smoke_Test_Report_Env E.html','<p>1. Account Creation and Purchasing a product: Successful Transaction as a new Member.<span style="color: green;">Test is Passed.</p>' + '\r\n' , {flag: 'a+'})
      }
    })
